import os
import time

ship_requirement = 10
damage_requirement = 1000


def get_ships(data):
    return int(data.split("producing ")[1].split(" ships")[0])

def get_damage(data):
    return int(data.split("dealing ")[1].split(" damage")[0])

def get_rank(data):
    return int(data.split("rank #")[1].split(" and")[0])

player_1_wins = 0
player_2_wins = 0

for num in range(5000):
    try:
        print("Currently on: {}".format(num))
        if player_1_wins > 0 or player_2_wins > 0:
            p1_pct = round(player_1_wins/(player_1_wins+player_2_wins)*100.0, 2)
            p2_pct = round(player_2_wins/(player_1_wins+player_2_wins)*100.0, 2)
            print("Player 1 win: {}%; Player 2 win: {}%.".format(p1_pct, p2_pct))

        os.system('halite.exe -d "360 240" "python MyBot-Python.py" "python MyBot-Python.py" >> data.gameout')

        with open('data.gameout', 'r') as f:
            contents = f.readlines()
            MLBot1 = contents[-4]
            MLBot2 = contents[-3]
            print(MLBot1)
            print(MLBot2)

            MLBot1_ships = get_ships(MLBot1)
            MLBot1_dmg = get_damage(MLBot1)
            MLBot1_rank = get_rank(MLBot1)

            MLBot2_ships = get_ships(MLBot2)
            MLBot2_dmg = get_damage(MLBot2)
            MLBot2_rank = get_rank(MLBot2)

            print("Charles1 rank: {} ships: {} dmg: {}".format(MLBot1_rank,MLBot1_ships,MLBot1_dmg))
            print("Charles2 rank: {} ships: {} dmg: {}".format(MLBot2_rank,MLBot2_ships,MLBot2_dmg))

        if MLBot1_rank == 1:
            print("c1 won")
            player_1_wins += 1
            if MLBot1_ships >= ship_requirement and MLBot1_dmg >= damage_requirement:
                with open("c1_input.vec","r") as f:
                    input_lines = f.readlines()
                with open("train.in","a") as f:
                    for l in input_lines:
                        f.write(l)

                with open("c1_out.vec","r") as f:
                    output_lines = f.readlines()
                with open("train.out","a") as f:
                    for l in output_lines:
                        f.write(l)

        elif MLBot2_rank == 1:
            print("c2 won")
            player_2_wins += 1
            if MLBot2_ships >= ship_requirement and MLBot2_dmg >= damage_requirement:
                with open("c2_input.vec","r") as f:
                    input_lines = f.readlines()
                with open("train.in","a") as f:
                    for l in input_lines:
                        f.write(l)
                    
                with open("c2_out.vec","r") as f:
                    output_lines = f.readlines()
                with open("train.out","a") as f:
                    for l in output_lines:
                        f.write(l)

        time.sleep(2)
    except Exception as e:
        print(str(e))
        time.sleep(2)